...
def main():
	rospy.init_node('ddpg_node')
	sess = tf.Session()
	K.set_session(sess)

	########################################################
	game_state= environment.Env()   # game_state has frame_step(action) function
	...
	if (train_indicator==1):
		...
		for i in range(num_trials):
			...
			for j in range(trial_len):
				...
				new_state,reward, done = game_state.step(action[0]) # we get reward and state here, then we need to calculate if it is crashed! for 'dones' value
